  ...
  methods: {
    async submit() {
      if(this.$refs.form.validate()) {
            ...
            const ref = this.$firebase.database().ref(`Users/${window.uid}/Dispositivos`) //o uid é a referencia
            const ref2 = this.$firebase.database().ref(`Dispositivos/`+ id + `/Users`)
        const payload = {
          nome,
          id
        }
        ref.child(id).set(payload)
        ref2.push(window.uid)
      }
    }
  },
}
